...
def newsrc(args):
    ...
    try:
        ...
        newSrc = classes.source(args.name, args.amount, args.interest)
        globalvar.masterSource[args.name] = newSrc
        globalvar.listStrSource += '\'{0}\' '.format(args.name)
        print('New source {0} created containing money amount of {1} with interest {2}.'.format(args.name, args.amount, args.interest))
